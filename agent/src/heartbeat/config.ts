...
const DEFAULT_HEARTBEAT_CONFIG: HeartbeatConfig = {
  entries: [
    ...
    {
      // Was 4-hourly. Changed to weekly per deployment requirements —
      // the root agent should notice + review upstream changes on a
      // weekly cadence, not check every few hours. Paired with an
      // explicit forced first-run at boot in index.ts (see
      // "run self-mod checks immediately on launch" there) — a cron
      // this infrequent would NOT reliably fire on a fresh install's
      // first tick otherwise: scheduler.ts's "never run, assume due"
      // fallback only backdates by 24h, which isn't enough headroom
      // for a 7-day cron to compute a next-run <= now.
    ...
    {
      // Added to defaults (previously defined as a task handler but
      // never actually scheduled by default — dormant unless an
      // operator manually added it via modify_heartbeat). Same weekly
      // cadence and same "won't reliably self-trigger on a fresh
      // install" caveat as check_for_updates above, same forced-first-
      // run fix in index.ts. Staggered 15 minutes after check_for_updates
      // so the two don't compete for the same tick.
